...
def get_same_or_newer(start_date):
    ...
    filename = "employees-with-date.csv"
    fields = []
    rows = []
    names = {}
    ...
    with open(filename) as csvfile:  
        ...
        for row in rows: 
            ...
            # employees at the minimal date.
            # if row_date == min_date:
            min_date_employees = ("{} {}".format(row[0], row[1]))

            names[row_date] = min_date_employees

    return names

def list_newer(start_date):
    names = get_same_or_newer(start_date)
    for x, y in names.items():
        while start_date < datetime.datetime.today():
            print("{} =>{}".format(start_date, y))
